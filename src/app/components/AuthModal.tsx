...
import { X, Mail, Lock, Eye, EyeOff, Gem, AlertCircle } from 'lucide-react';
...
export function AuthModal() {
  const { login, loginWithGoogle, isLoading, showAuthModal, setShowAuthModal, authRedirectPlanId, setAuthRedirectPlanId } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userId = `user-email-${email}`;
      await handlePostLogin(userId, email);
    } catch (err: any) {
      setError(err.message || (isAr ? 'حدث خطأ. حاول مرة أخرى.' : 'An error occurred. Please try again.'));
    }
  };
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      await handlePostLogin('google-user', 'user@gmail.com');
    } catch (err: any) {
      ...
      >
        ...
        <div className="px-6 pt-6 pb-8 -mt-4 relative z-10">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-200 text-gray-700 font-semibold rounded-xl px-4 py-3 hover:border-blue-300 hover:bg-blue-50 transition-all disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold rounded-xl py-3 transition-all disabled:opacity-60 disabled:cursor-not-allowed shadow-lg hover:shadow-blue-200 hover:scale-[1.02] active:scale-[0.98]"
            >
              {isLoading
